...
1. **Metadata Array (Static Array):**
      ...
      * **Contents:** `pc` (64-bit program counter), `is_rvc` (1-bit compressed instruction flag), `rd_addr` (5-bit), `rd_type` (1-bit), `rd_we` (1-bit), `op_class` (4-bit `op_class_t`) encapsulated as `rb_static_meta_t`.
